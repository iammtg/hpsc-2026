...
int main(int argc, char** argv) {
  ...
  for(int i=0; i<N/size; i++) {
    ...
    ibody[i].fx = jbody[i].fx = kbody[i].fx = 0;
    ibody[i].fy = jbody[i].fy = kbody[i].fy = 0;
  }
  ...
  for(int irank=0; irank<size; irank++) {
    MPI_Win_fence(0, win);
    MPI_Put(kbody, N/size, MPI_BODY, send_to, 0, N/size, MPI_BODY, win);
    MPI_Win_fence(0, win);
    for(int i=0; i<N/size; i++) kbody[i] = jbody[i];
    for(int i=0; i<N/size; i++) {
